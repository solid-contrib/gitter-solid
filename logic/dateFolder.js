...
export class DateFolder {
  constructor (rootThing, leafFileName, membershipProperty, store) {
    ...
    this.kb = store
    if (!this.kb) {
      this.kb = new $rdf.Store()
      new $RDF.Fetcher(this.kb)
    }
  }
  ...
  async loadPrevious (date, backwards) {
    ...
    async function previousPeriod (file, level) {
      ...
      async function lastNonEmpty (siblings) {
        ...
        while (siblings.length) {
          ...
          const leafDocument = $rdf.sym(folder.uri + thisDateFolder.leafFileName)
          await thisKb.fetcher.load(leafDocument)
          // files can have seealso links. skip ones with no leafObjects with a date
          if (
            thisKb.statementsMatching(null, ns.dct('created'), null, leafDocument)
              .length > 0
          ) {
            return folder
          }
      ...
      await thisKb.fetcher.load(parent)
      var siblings = thisKb.each(parent, ns.ldp('contains'))
      ...
      await this.kb.fetcher.load(uncle)
      var cousins = this.kb.each(uncle, ns.ldp('contains'))
    ...
    if (found) {
      const doc = $rdf.sym(found.uri + this.leafFileName)
